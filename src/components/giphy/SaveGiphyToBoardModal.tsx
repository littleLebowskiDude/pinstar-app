...
import { saveGiphyPin } from '@/app/actions/pins'
import CreateBoardModal from '@/components/boards/CreateBoardModal'
import { useToast } from '@/components/ui/ToastContext'
import Image from 'next/image'

interface SaveGiphyToBoardModalProps {
  isOpen: boolean
  onClose: () => void
  gif: GiphyImage | null
  onSuccess?: (gifId: string) => void
}

export default function SaveGiphyToBoardModal({
  isOpen,
  onClose,
  gif,
  onSuccess,
}: SaveGiphyToBoardModalProps) {
  ...
  // Pre-fill title when gif changes
  useEffect(() => {
    if (gif) {
      setTitle(gif.title || 'Untitled GIF')
      setDescription('')
    }
  }, [gif])
  ...
  const handleSaveToBoard = async (boardId: string) => {
    ...
    try {
      const result = await saveGiphyPin({
        giphyImage: gif,
        boardId,
      ...
      if (result.success) {
        ...
        showSuccess(`GIF saved to ${board?.name || 'board'}!`)
        router.refresh()
        if (onSuccess) {
          onSuccess(gif.id)
        }
        ...
        showError(result.error || 'Failed to save GIF')
      }
    } catch (error) {
      console.error('Error saving GIF:', error)
      showError('Failed to save GIF')
    } finally {
      setIsSaving(false)
      setSelectedBoardId(null)
    }
      ...
      >
        <div className="relative bg-white dark:bg-gray-900 rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col animate-scaleIn">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Save GIF to Board</h2>
            <button
              onClick={onClose}
              disabled={isSaving}
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="mb-4">
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition-all dark:bg-gray-700 dark:text-white dark:placeholder-gray-400"
                maxLength={200}
              />
            </div>
            ...
            <div className="mb-6">
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition-all resize-none dark:bg-gray-700 dark:text-white dark:placeholder-gray-400"
                maxLength={1000}
              />
            </div>
            ...
            <div>
              ...
              {isLoading ? (
                ...
                <div className="text-center py-8 bg-gray-50 dark:bg-gray-800 rounded-xl">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Create a board to save this GIF</p>
                </div>
              ) : (
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {boards.map((board: any) => (
                    ...
                    >
                      ...
                      {selectedBoardId === board.id && isSaving && (
                        <div className="w-5 h-5 animate-spin rounded-full border-2 border-solid border-purple-600 border-r-transparent"></div>
                      )}
                    </button>
                  ))}
                </div>
